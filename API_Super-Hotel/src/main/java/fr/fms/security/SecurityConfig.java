...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
        http.formLogin()
                .loginPage("/login").usernameParameter("mail").passwordParameter("password").defaultSuccessUrl("/")
                .failureUrl("/login?error=true").permitAll();

        http.logout().logoutUrl("/logout").logoutSuccessUrl("/login");

        //TODO mettre les bonnes URL
        http.authorizeRequests().antMatchers("/city").hasRole("ADMIN");
        //TODO mettre les bonnes URL
        http.authorizeRequests().antMatchers("/hotel").hasRole("GESTIONNAIRE");
